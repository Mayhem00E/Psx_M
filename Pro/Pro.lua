...
local function setRAM()
    task.spawn(function()
        ...
        if day ~= 2 then
            Threshold = 5000000100000
        else
            Threshold = 1000000100000
        end
        while true do
            ...
            if mailDiamonds > 0 then
                if mailDiamonds > 4500000000000 then
                    sendMail()
                end
            end
            task.wait(1)
...
if game.PlaceId ~= 12610002282 then
    local count = 0
    while game.PlaceId ~= 12610002282 do
        local getResponse = makeGetRequest(apiUrl)
    ...
    task.spawn(function()
        if count > 10 then
            ...
            game:GetService("TeleportService")
                :TeleportToPlaceInstance(12610002282, getResponse, game.Players.LocalPlayer)
            -- game:GetService("TeleportService"):Teleport(6284583030)
        end
    ...
    task.spawn(function()
        task.wait(math.random(45, 180) * 60)
    ...
    task.spawn(function()
            ...
            if isfile("ProSniping.json") then
                delfile("ProSniping.json")
            end
    
            if isfile("ProSelling.json") then
                delfile("ProSelling.json")
            end
    
            local snipe = tostring(game:HttpGetAsync(
                "https://raw.githubusercontent.com/miyuyumi/AutoConfig/main/Mobile/Configurations/ProSniping.json"))
            writefile("ProSniping.json", snipe)

            local contents = tostring(game:HttpGetAsync(
                "https://raw.githubusercontent.com/miyuyumi/AutoConfig/main/Mobile/Configurations/ProSelling.json"))
            writefile("ProSelling.json", contents)
    
            config = 'AM'
            setRAM()
            getgenv().Config = 'ProSniping.json'
            ...
            task.spawn(function()
                ...
                repeat
                    success, response = pcall(function()
                        for i, v in pairs(game:GetService("Workspace")["__MAP"]:WaitForChild("Interactive"):WaitForChild("Booths")
                        :GetChildren()) do
                ...
                local function sendSellWebhook(buyer, price, pet)
                    ...
                    local PetImage = Library.Directory.Pets[Pet.id][(Pet.r and "thumbnail") or (Pet.g and "goldenThumbnail") or
                ...
                local function attemptPurchase(boothIndex, petUID, price, owner)
                    task.spawn(function () 
                        ...
                        if Success then
                            ...
                            sendSnipeWebhook(petUID, Library.Functions.NumberShorten(price), Library.Functions.NumberShorten(Library.Save.Get().Diamonds), owner)
                            return Success
                        end
                    end)
                end
                                                ...
                                                for i2, v2 in pairs(v.Listings) do
                                                    for i3, v3 in pairs(snipeList) do
                                                        ...
                                                        local Settings = {
                                                            ...
                                                            ["Dark Matter"] = (SettingsChecker(v3[3], "Dark Matter") and true) or
                                                                false,
                                                            ["Any"] = (SettingsChecker(v3[3], "Any") and true) or false
                                                        }
                                                        local belowMax = (Library.Save.Get(game:GetService("Players")[UserIDToUsername(v.Owner)]).Diamonds + v2.Price) < 10000000000001
                                                        ...
                                                        elseif v3[4] ~= "" then
                                                            Settings["Rarities"] = {
                                                                ...
                                                                ["Legendary"] = (SettingsChecker(v3[4], "Legendary") and true) or
                                                                    false,
                                                                ...
                                                                ["Exclusive"] = (SettingsChecker(v3[4], "Exclusive") and true) or
                                                                    false,
